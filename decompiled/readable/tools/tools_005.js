...
function nb1(A) {
    ...
    for (let V of Q.allow || []) {
        let K = AUA(V);
        if (K.toolName === WEB_FETCH_TOOL_NAME && K.ruleContent?.startsWith("domain:")) B.push(K.ruleContent.substring(7))
    }
    for (let V of Q.deny || []) {
        let K = AUA(V);
        if (K.toolName === WEB_FETCH_TOOL_NAME && K.ruleContent?.startsWith("domain:")) G.push(K.ruleContent.substring(7))
    }
    let Z = ["."],
        I = [],
        Y = [],
    ...
    for (let V of Q.allow || []) {
        let K = AUA(V);
        if (K.toolName === EDIT_TOOL_NAME && K.ruleContent) Z.push(K.ruleContent)
    }
    for (let V of Q.deny || []) {
        let K = AUA(V);
        if (K.toolName === EDIT_TOOL_NAME && K.ruleContent) I.push(K.ruleContent);
        if (K.toolName === READ_TOOL_NAME && K.ruleContent) Y.push(K.ruleContent)
    }
...
function y06() {
    ...
    try {
        ...
        for (let I of [...B.allow || [], ...B.deny || []]) {
            let Y = AUA(I);
            if ((Y.toolName === EDIT_TOOL_NAME || Y.toolName === READ_TOOL_NAME) && Y.ruleContent && Z(Y.ruleContent)) G.push(I)
        }
        return G
    } catch (Q) {
...
function o7B(A, Q) {
    let B = LB("localSettings"),
        ...
        Z = A;
    if (Q) {
        let I = Q.filter((Y) => Y.type === "addRules" && Y.rules.some((J) => J.toolName === BASH_TOOL_NAME));
        if (I.length > 0 && I[0].type === "addRules") {
            let Y = I[0].rules.find((J) => J.toolName === BASH_TOOL_NAME);
