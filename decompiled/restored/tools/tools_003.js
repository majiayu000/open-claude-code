 ...
 *
 * 原始文件: tools_003.js
 * 处理时间: 2025-12-09T03:37:25.527Z
 *
 * 注意: 此文件保留了100%的原始代码逻辑
 * 通过注释解释混淆的变量名和函数名
 *
 * ==================== 变量索引 ====================
 * hN         (  4x) = copyObject() - Copy object properties
 * EvA        (  3x) = baseClone() - Base clone
 * HvA        (  3x) = getAllKeysIn() - Get all keys including inherited
 * HV         (  2x) = Symbol - Symbol reference
 * SN         (  2x) = keys() - Get object keys
 * o9         (  1x) = getConfig() - Returns config with BASE_API_URL, OAuth endpoi
 ...
 * j7         (  1x) = isArray() - Check if array
 * jBA        (  1x) = arrayPush() - Array push helper
 * ===================================================
 */
...
var EX = L(() => {
    ...
    D34 = [K34, "user:profile"], SC1 = ["user:profile", KvA, "user:sessions:claude_code"], Ul0 = Array.from(new Set([...D34, ...SC1])), Cl0 = {
        BASE_API_URL: /* ANTHROPIC_API_BASE_URL */ "https://api.anthropic.com",
