...
var kt2 = U((_t2) => {
    ...
    function JE3(A) {
        let Q = TY0.resolve(A);
        if (!AZ1.existsSync(Q)) throw Error(`Cannot read contents of ${Q}. Directory does not exist.`);
        if (!AZ1.statSync(Q).isDirectory()) throw Error(`Cannot read contents of ${Q}, because it is not a directory.`);
        let B = (G) => {
...
var vt2 = "Anr",
        ...
        qE3 = Dg.convertIntegrationFnToClass(vt2, bt2);
    async function NE3(A, Q) {
        ...
        let {
            Worker: J
        } = UE3(), W = new J(new KE3.URL(`data:application/javascript;base64,${DE3.base64WorkerScript}`), {
            workerData: Y
        });
        process.on("exit", () => {
            W.terminate()
...
var SY0 = U((ct2) => {
    ...
    function jE3(A = {}) {
        return function({
            ...
            if (Y) {
                Y.updateName(`trpc/${Q}`), Y.setAttribute(UXA.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "route"), Y.op = "rpc.server";
                let X = {
                    procedure_type: B
                };
    ...
    function gz3(A, Q) {
        ...
        GQA.withScope((G) => {
            if (G.addEventProcessor((Y) => {
                ...
                let Y = `Assertion failed: ${ZQA.safeJoin(A.slice(1)," ")||"console.assert"}`;
