 ...
 *
 * 原始文件: prompts_010.js
 * 处理时间: 2025-12-09T03:37:25.323Z
 *
 * 注意: 此文件保留了100%的原始代码逻辑
 * 通过注释解释混淆的变量名和函数名
 *
 * ==================== 变量索引 ====================
 * GA         (  8x) = esmImport(module) - ESM import helper
 * D9         (  6x) = BASH_TOOL = "Bash"
 * en         (  2x) = AGENT_OUTPUT_TOOL = "AgentOutputTool"
 * S3         (  2x) = getDefaultSonnetModel() - Returns main Claude model name
 * ===================================================
 */
...
function $J1({
    ...
    return TC.default.createElement(TC.default.Fragment, null, TC.default.createElement(j, {
        ...
        bold: !0
    }, /* ENV_API_KEY */ "ANTHROPIC_API_KEY"), TC.default.createElement($, null, ": sk-ant-...", A)), TC.default.createElement($, null, "Do you want to use this API key?"), TC.default.createElement(M0, {
        defaultValue: "no",
        focusValue: "no",
        options: [{
            label: "Yes",
...
function Zz9(A) {
    return A.some((Q) => Q.ruleBehavior === "allow" && (Q.ruleValue.toolName === D9 || Q.ruleValue.toolName.startsWith(D9 + "(")))
}
...
function zz9({
    ...
    let {
        servers: B
    } = yX("project"), G = ZI("trust_folder_dialog_copy", "variant", "control"), Z = Hz9[G], I = Object.keys(B).length > 0, Y = Wz9(), J = Y.length > 0, W = Xz9(), X = Vz9(), F = X.length > 0, V = Kz9(), K = V.length > 0, D = Fz9(), H = D.length > 0, C = [...new Set([...Y, ...W, ...X, ...V, ...D])], E = Q?.filter((d) => d.type === "prompt" && d.source === "projectSettings" && !d.isSkill && d.allowedTools?.some((QA) => QA === D9 || QA.startsWith(D9 + "("))) ?? [], z = Q?.filter((d) => d.type === "prompt" && d.source === "localSettings" && d.isSkill && d.allowedTools?.some((QA) => QA === D9 || QA.startsWith(D9 + "("))) ?? [], w = E.length > 0, N = z.length > 0, q = E.map((d) => d.name), R = z.map((d) => d.name), P = W.length > 0 || w || N, y = _X(J || P || F || K || H), x = [{
        name: "MCP servers",
        shouldShowWarning: () => I,
        onChange: () => {
            let d = {
