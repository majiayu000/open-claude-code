 ...
 *
 * ===================== 变量索引 =====================
 * M8       ( 15x) shellEscape(args) - Escape shell args
 * D9       (  6x) BASH_TOOL = "Bash"
 * BwA      (  6x) fs.existsSync
 * GA       (  5x) esmImport(module) - ESM import
 * id1      (  4x) hasHeredoc(cmd) - Check heredoc
 * nd1      (  4x) isOperator(token, op) - Check operator
 * sd1      (  4x) path.join
 * rd1      (  4x) getShellRcFile() - Get .bashrc/.zshrc
 * RJ       (  4x) isClaudeCodeRemote() - Check remote
 ...
 * TSB      (  3x) extractTokensRange() - Extract tokens
 * DoA      (  3x) os module
 * s8       (  2x) TASK_TOOL = "Task"
 * tI       (  2x) TODO_READ_TOOL object
 * NSB      (  2x) getReadToolDescription() - Read tool description
 * oM6      (  2x) getSandboxHint() - Returns sandbox usage hint
 * tM6      (  2x) getGitCommitInstructions() - Git commit instructions
 * rM6      (  2x) getGitConfig() - Git commit/pr config object
 * eM6      (  2x) hasMultilineString(cmd) - Check multiline
 * AO6      (  2x) hasInputRedirect(cmd) - Check redirect
 * BO6      (  2x) isEnvAssignment(token) - Check env assign
 * GO6      (  2x) isCommandSeparator(op) - Check separator
 * QO6      (  2x) findPipeIndex(tokens) - Find pipe
 * OSB      (  2x) shouldAddDevNull(cmd) - Check /dev/null need
 * MSB      (  2x) wrapBashCommand(cmd) - Wrap command
 * PSB      (  2x) addDevNullToCommand(cmd) - Add /dev/null
 * RSB      (  2x) bashCommandHelpers loader
 * ZO6      (  2x) fs.statSync
 * IO6      (  2x) fs.mkdirSync
 * YO6      (  2x) fs.realpathSync
 * JO6      (  2x) child_process.execSync
 * WO6      (  2x) child_process.execFile
 * XO6      (  2x) getRipgrepCommand() - Get rg command
 * FO6      (  2x) getShellSnapshotScript()
 * CD       (  1x) GLOB_TOOL = "Glob"
 * uY       (  1x) GREP_TOOL = "Grep"
 * g5       (  1x) READ_TOOL = "Read"
 * R5       (  1x) EDIT_TOOL = "Edit"
 * bX       (  1x) WRITE_TOOL = "Write"
 * LSB      (  1x) getBashToolDescription() - Bash tool description
 * XT       (  1x) noOpFunction() - Empty function
 * IW       (  1x) tokenize(cmd) - Tokenize command
 * C9A      (  1x) getRipgrepConfig() - Get rg config
