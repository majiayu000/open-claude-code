 ...
 * Category: ui
 * File: 53/53
 * Lines: 384323 - 385822 (1500 lines)
 ...
 */
        ...
        let B = {
            ...A,
            stackParser: QQA.stackParserFromStackParserOptions(A.stackParser || jt2),
            integrations: TO.getIntegrationsToSetup(A),
            transport: A.transport || rC3.makeNodeTransport
        };
        if (TO.initAndBind(A.clientClass || hC3.NodeClient, B), A.autoSessionTracking) eC3();
        if (AE3(), A.spotlight) {
            let G = TO.getClient();
            if (G && G.addIntegration) {
                let Z = G.getOptions().integrations;
                for (let I of Z) G.addIntegration(I);
                G.addIntegration(nC3.spotlightIntegration({
    ...
    function Pt2(A) {
        ...
        if (QQA.GLOBAL_OBJ.SENTRY_RELEASE && QQA.GLOBAL_OBJ.SENTRY_RELEASE.id) return QQA.GLOBAL_OBJ.SENTRY_RELEASE.id;
    ...
    var jt2 = QQA.createStackParser(QQA.nodeStackLineParser(sC3.createGetModuleFromFilename()));

    function eC3() {
        TO.startSession(), process.on("beforeExit", () => {
            let A = TO.getIsolationScope().getSession();
            if (A && !["exited", "crashed"].includes(A.status)) TO.endSession()
        })
    }
...
var BZ1 = moduleWrapper((ft2, ht2) => {
    ...
    function jY0(A, ...Q) {
        QZ1.logger.log(`[ANR] TextComponent{A}`, ...Q)
    }

    function EE3() {
        return QZ1.GLOBAL_OBJ
    }

    function zE3() {
        let A = Dg.getGlobalScope().getScopeData();
        return Dg.mergeScopeData(A, Dg.getIsolationScope().getScopeData()), Dg.mergeScopeData(A, Dg.getCurrentScope().getScopeData()), A.attachments = [], A.eventProcessors = [], A
    }

    function UE3() {
        return QZ1.dynamicRequire(ht2, "worker_threads")
    }
    async function $E3(A) {
        let Q = {
                message: "ANR"
        ...
        return VE3([Q, "optionalAccess", (G) => G.contexts]) || {}
    }
    /* ANR_MONITOR = ANR_MONITOR = "Anr" */
var ANR_MONITOR = "Anr",
        wE3 = (A = {}) => {
            if (PY0.NODE_VERSION.major < 16 || PY0.NODE_VERSION.major === 16 && PY0.NODE_VERSION.minor < 17) throw Error("ANR detection requires Node 16.17.0 or later");
            let Q, B, G = EE3();
            return G.__SENTRY_GET_SCOPES__ = zE3, {
                name: ANR_MONITOR,
                setupOnce() {},
                startWorker: () => {
                    if (Q) return;
        ...
        bt2 = Dg.defineIntegration(wE3),
        qE3 = Dg.convertIntegrationFnToClass(ANR_MONITOR, bt2);
    async function NE3(A, Q) {
        let B = A.getDsn();
        if (!B) return () => {};
        let G = await $E3(A);
        xt2([G, "access", (F) => F.app, "optionalAccess", (F) => delete F.app_memory]), xt2([G, "access", (F) => F.device, "optionalAccess", (F) => delete F.free_memory]);
        let Z = A.getOptions(),
            I = A.getSdkMetadata() || {};
        if (I.sdk) I.sdk.integrations = Z.integrations.map((F) => F.name);
        let Y = {
            debug: QZ1.logger.isEnabled(),
            dsn: B,
            ...
            pollInterval: Q.pollInterval || HE3,
            anrThreshold: Q.anrThreshold || CE3,
            captureStackTrace: !!Q.captureStackTrace,
            staticTags: Q.staticTags || {},
            contexts: G
        };
        if (Y.captureStackTrace) {
            let F = nodeRequire("inspector");
            if (!F.url()) F.open(0)
        }
        let {
            Worker: J
        } = UE3(), W = new J(new KE3.URL(`data:application/javascript;base64,TextComponent{DE3.base64WorkerScript}`), {
            workerData: Y
        });
        process.on("exit", () => {
            W.terminate()
        ...
        let X = setInterval(() => {
            try {
                let F = Dg.getCurrentScope().getSession(),
                    V = F ? {
                        ...F,
                        toJSON: void 0
                    } : void 0;
        ...
        return X.unref(), W.on("message", (F) => {
            if (F === "session-ended") jY0("ANR event sent from ANR worker. Clearing session in this thread."), Dg.getCurrentScope().setSession(void 0)
        }), W.once("error", (F) => {
            clearInterval(X), jY0("ANR worker error", F)
        }), W.once("exit", (F) => {
            clearInterval(X), jY0("ANR worker exit", F)
        }), W.unref(), () => {
            W.terminate(), clearInterval(X)
        }
    }
    ft2.Anr = qE3;
    ft2.anrIntegration = bt2
});
var ut2 = moduleWrapper((gt2) => {
    Object.defineProperty(gt2, "__esModule", {
        value: !0
    });
    var OE3 = P4(),
        RE3 = BZ1();
    ...
    function TE3(A) {
        let Q = OE3.getClient();
        return new RE3.Anr(A).setup(Q), Promise.resolve()
    }
    gt2.enableAnrDetection = TE3
});
var SY0 = moduleWrapper((ct2) => {
    var {
        _optionalChain: mt2
    } = l0();
    ...
    function jE3(A = {}) {
        return function({
            ...
            let I = mt2([UXA.getClient, "call", (X) => X(), "optionalAccess", (X) => X.getOptions, "call", (X) => X()]),
                Y = UXA.getCurrentScope().getTransaction();
            if (Y) {
                Y.updateName(`trpc/TextComponent{Q}`), Y.setAttribute(UXA.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, "route"), Y.op = "rpc.server";
                let X = {
                    procedure_type: B
                };
                if (A.attachRpcInput !== void 0 ? A.attachRpcInput : mt2([I, "optionalAccess", (F) => F.sendDefaultPii])) X.input = dt2.normalize(Z);
                Y.setContext("trpc", X)
            }

            function J(X) {
                if (!X.ok) UXA.captureException(X.error, {
                    mechanism: {
                        handled: !1,
                        data: {
            ...
            if (dt2.isThenable(W)) Promise.resolve(W).then((X) => {
                J(X)
            }, (X) => {
                UXA.captureException(X, {
                    mechanism: {
                        handled: !1,
                        data: {
                            function: "trpcMiddleware"
    ...
    ct2.trpcMiddleware = jE3
});
var it2 = moduleWrapper((lt2) => {
    ...
    function _E3(A, Q) {
        return pt2.extractRequestData(A, {
            include: Q
        })
    }

    function kE3(A, Q, B = {}) {
        return pt2.addRequestDataToEvent(A, Q, {
            include: B
        })
    }
    lt2.extractRequestData = _E3;
    lt2.parseRequest = kE3
});
var st2 = moduleWrapper((at2) => {
    var {
        _optionalChain: GZ1
    } = l0();
    ...
    function fE3() {
        return function(Q, B, G) {
            let Z = GZ1([HK.getClient, "call", (F) => F(), "optionalAccess", (F) => F.getOptions, "call", (F) => F()]);
            if (!Z || Z.instrumenter !== "sentry" || GZ1([Q, "access", (F) => F.method, "optionalAccess", (F) => F.toUpperCase, "call", (F) => F()]) === "OPTIONS" || GZ1([Q, "access", (F) => F.method, "optionalAccess", (F) => F.toUpperCase, "call", (F) => F()]) === "HEAD") return G();
            let I = Q.headers && $XA.isString(Q.headers["sentry-trace"]) ? Q.headers["sentry-trace"] : void 0,
                Y = GZ1([Q, "access", (F) => F.headers, "optionalAccess", (F) => F.baggage]);
            if (!HK.hasTracingEnabled(Z)) return G();
            let [J, W] = $XA.extractPathForTransaction(Q, {
                path: !0,
                method: !0
            }), X = HK.continueTrace({
                sentryTrace: I,
                baggage: Y
            }, (F) => HK.startTransaction({
                name: J,
                op: "http.server",
                origin: "auto.http.node.tracingHandler",
                ...F,
                data: {
                    [HK.SEMANTIC_ATTRIBUTE_SENTRY_SOURCE]: W
                },
                metadata: {
                    ...F.metadata,
                    request: Q
                ...
                request: $XA.extractRequestData(Q)
            }));
            HK.getCurrentScope().setSpan(X), B.__sentry_transaction = X, B.once("finish", () => {
                setImmediate(() => {
                    $XA.addRequestDataToTransaction(X, Q), HK.setHttpStatus(X, B.statusCode), X.end()
                })
            }), G()
        }
    }
    ...
    function gE3(A) {
        let Q = hE3(A),
            B = HK.getClient();
        if (B && ZZ1.isAutoSessionTrackingEnabled(B)) {
            B.initSessionFlusher();
            let G = HK.getCurrentScope();
            if (G.getSession()) G.setSession()
        }
        return function(Z, I, Y) {
            if (A && A.flushTimeout && A.flushTimeout > 0) {
                let J = I.end;
                I.end = function(W, X, F) {
                    HK.flush(A.flushTimeout).then(() => {
                        J.call(this, W, X, F)
                    }).then(null, (V) => {
                        vE3.DEBUG_BUILD && $XA.logger.error(V), J.call(this, W, X, F)
                    })
                }
            }
            HK.runWithAsyncContext(() => {
                let J = HK.getCurrentScope();
                J.setSDKProcessingMetadata({
                    request: Z,
                    requestDataOptionsFromExpressHandler: Q
                });
                let W = HK.getClient();
                if (ZZ1.isAutoSessionTrackingEnabled(W)) J.setRequestSession({
                    status: "ok"
                });
                I.once("finish", () => {
                    let X = HK.getClient();
                    if (ZZ1.isAutoSessionTrackingEnabled(X)) setImmediate(() => {
    ...
    function dE3(A) {
        return function(B, G, Z, I) {
            if ((A && A.shouldHandleError || mE3)(B)) {
                HK.withScope((J) => {
                    J.setSDKProcessingMetadata({
                        request: G
                    });
                    let W = Z.__sentry_transaction;
                    if (W && !HK.getActiveSpan()) J.setSpan(W);
                    let X = HK.getClient();
                    if (X && ZZ1.isAutoSessionTrackingEnabled(X)) {
                        if (X._sessionFlusher !== void 0) {
                            let K = J.getRequestSession();
                            if (K && K.status !== void 0) K.status = "crashed"
                        }
                    }
                    let F = HK.captureException(B, {
                        mechanism: {
                            type: "middleware",
                            handled: !1
                        }
    ...
    var cE3 = bE3.trpcMiddleware;
    at2.extractRequestData = nt2.extractRequestData;
    at2.parseRequest = nt2.parseRequest;
    at2.errorHandler = dE3;
    at2.requestHandler = gE3;
    at2.tracingHandler = fE3;
    at2.trpcMiddleware = cE3
});
var _Y0 = moduleWrapper((Be2) => {
    ...
    var tt2 = {
            ...
            version: q$.SDK_VERSION,
            register: async function(A) {
                A.events.on("request", (B, G) => {
                    let Z = q$.getActiveTransaction();
                    if (rE3(G)) oE3(G.error);
        ...
        et2 = {
            ...
            register: async function(A) {
                ...
                Q.ext("onPreHandler", (B, G) => {
                    let Z = q$.continueTrace({
                        sentryTrace: B.headers["sentry-trace"] || void 0,
                        baggage: B.headers.baggage || void 0
                    }, (I) => {
                        return q$.startTransaction({
                            ...I,
                            op: "hapi.request",
                            name: B.route.path,
                            description: `TextComponent{B.route.method} TextComponent{B.path}`
                        })
                    });
                    return q$.getCurrentScope().setSpan(Z), G.continue
                }), Q.ext("onPreResponse", (B, G) => {
                    let Z = q$.getActiveTransaction();
                    if (B.response && rt2(B.response) && Z) {
                        let I = B.response;
                        I.header("sentry-trace", q$.spanToTraceHeader(Z));
                        let Y = ot2.dynamicSamplingContextToSentryBaggageHeader(q$.getDynamicSamplingContextFromSpan(Z));
                    ...
                    let Z = q$.getActiveTransaction();
                    if (Z) {
                        if (B.response && rt2(B.response)) q$.setHttpStatus(Z, B.response.statusCode);
                        Z.end()
                    }
                    return G.continue
                })
                ...
                setupOnce() {
                    ...
                    ot2.fill(Q, "start", (B) => {
                        return async function() {
                            return await this.register(et2), await this.register(tt2), B.apply(this)
                        }
                    })
                }
            }
        },
        Qe2 = q$.defineIntegration(tE3),
        eE3 = q$.convertIntegrationFnToClass(Ae2, Qe2);
    Be2.Hapi = eE3;
    Be2.hapiErrorPlugin = tt2;
    Be2.hapiIntegration = Qe2;
    Be2.hapiTracingPlugin = et2
});
var Ze2 = moduleWrapper((Ge2) => {
    ...
    var Zz3 = fG1(),
        ...
        Cz3 = BZ1(),
        Ez3 = _Y0();
    Ge2.Console = Zz3.Console;
    Ge2.Http = Iz3.Http;
    Ge2.OnUncaughtException = Yz3.OnUncaughtException;
    Ge2.OnUnhandledRejection = Jz3.OnUnhandledRejection;
    Ge2.Modules = Wz3.Modules;
    Ge2.ContextLines = Xz3.ContextLines;
    Ge2.Context = Fz3.Context;
    Ge2.RequestData = Vz3.RequestData;
    Ge2.LocalVariables = Kz3.LocalVariables;
    Ge2.Undici = Dz3.Undici;
    Ge2.Spotlight = Hz3.Spotlight;
    Ge2.Anr = Cz3.Anr;
    Ge2.Hapi = Ez3.Hapi
});
var Ye2 = moduleWrapper((Ie2) => {
    Object.defineProperty(Ie2, "__esModule", {
        value: !0
    });
    var BQA = XY0();
    Ie2.Apollo = BQA.Apollo;
    Ie2.Express = BQA.Express;
    Ie2.GraphQL = BQA.GraphQL;
    Ie2.Mongo = BQA.Mongo;
    Ie2.Mysql = BQA.Mysql;
    Ie2.Postgres = BQA.Postgres;
    Ie2.Prisma = BQA.Prisma
});
var Fe2 = moduleWrapper((Xe2) => {
    ...
    var GQA = P4(),
        ...
        Je2 = "CaptureConsole",
        fz3 = (A = {}) => {
            let Q = A.levels || ZQA.CONSOLE_LEVELS;
            return {
                name: Je2,
                setupOnce() {},
                setup(B) {
                    if (!("console" in ZQA.GLOBAL_OBJ)) return;
                    ZQA.addConsoleInstrumentationHandler(({
                        args: G,
                        level: Z
                    }) => {
                        if (GQA.getClient() !== B || !Q.includes(Z)) return;
                        gz3(G, Z)
                    })
                }
            }
        },
        We2 = GQA.defineIntegration(fz3),
        hz3 = GQA.convertIntegrationFnToClass(Je2, We2);

    function gz3(A, Q) {
        let B = {
            level: ZQA.severityLevelFromString(Q),
            extra: {
                arguments: A
            }
        };
        GQA.withScope((G) => {
            if (G.addEventProcessor((Y) => {
                    return Y.logger = "console", ZQA.addExceptionMechanism(Y, {
                        handled: !1,
                        type: "console"
                    }), Y
                }), Q === "assert" && A[0] === !1) {
                let Y = `Assertion failed: TextComponent{ZQA.safeJoin(A.slice(1)," ")||"console.assert"}`;
                G.setExtra("arguments", A.slice(1)), GQA.captureMessage(Y, B);
                return
            }
            let Z = A.find((Y) => Y instanceof Error);
            if (Q === "error" && Z) {
                GQA.captureException(Z, B);
                return
            }
            let I = ZQA.safeJoin(A, " ");
            GQA.captureMessage(I, B)
        })
    }
    Xe2.CaptureConsole = hz3;
    Xe2.captureConsoleIntegration = We2
});
var Ce2 = moduleWrapper((He2) => {
    ...
    var Ve2 = P4(),
        ...
        De2 = Ve2.defineIntegration(cz3),
        pz3 = Ve2.convertIntegrationFnToClass(Ke2, De2);
    He2.Debug = pz3;
    He2.debugIntegration = De2
});
var RPA = moduleWrapper((Ee2) => {
    Object.defineProperty(Ee2, "__esModule", {
        value: !0
    });
    var nz3 = typeof __SENTRY_DEBUG__ > "u" || __SENTRY_DEBUG__;
    Ee2.DEBUG_BUILD = nz3
});
var Re2 = moduleWrapper((Oe2) => {
        ...
        oz3 = () => {
            ...
            return {
                ...
                processEvent(Q) {
                    if (Q.type) return Q;
                    try {
                        if (Ne2(Q, A)) return rz3.DEBUG_BUILD && sz3.logger.warn("Event dropped due to being a duplicate of previously captured event."), null
                    } catch (B) {}
                    return A = Q
                }
            }
        },
        qe2 = $e2.defineIntegration(oz3),
        tz3 = $e2.convertIntegrationFnToClass(we2, qe2);
    ...
    Oe2.Dedupe = tz3;
    Oe2._shouldDropEvent = Ne2;
    Oe2.dedupeIntegration = qe2
});
var _e2 = moduleWrapper((Se2) => {
    ...
    var Te2 = P4(),
        ...
        je2 = Te2.defineIntegration(IU3),
        YU3 = Te2.convertIntegrationFnToClass(Pe2, je2);

    function JU3(A, Q = {}, B, G) {
        if (!Q.originalException || !an.isError(Q.originalException)) return A;
        let Z = Q.originalException.name || Q.originalException.constructor.name,
            I = WU3(Q.originalException, G);
        if (I) {
            let Y = {
                    ...A.contexts
                },
                J = an.normalize(I, B);
            if (an.isPlainObject(J)) an.addNonEnumerableProperty(J, "__sentry_skip_normalization__", !0), Y[Z] = J;
            return {
                ...A,
                contexts: Y
            }
    ...
    function WU3(A, Q) {
        try {
            ...
            for (let Z of Object.keys(A)) {
                ...
                let I = A[Z];
                G[Z] = an.isError(I) ? I.toString() : I
            }
            if (Q && A.cause !== void 0) G.cause = an.isError(A.cause) ? A.cause.toString() : A.cause;
            if (typeof A.toJSON === "function") {
                let Z = A.toJSON();
                for (let I of Object.keys(Z)) {
                    let Y = Z[I];
                    G[I] = an.isError(Y) ? Y.toString() : Y
                }
            }
            return G
        } catch (B) {
            ZU3.DEBUG_BUILD && an.logger.error("Unable to extract extra data from the Error object:", B)
        }
        return null
    }
    Se2.ExtraErrorData = YU3;
    Se2.extraErrorDataIntegration = je2
});
var ye2 = moduleWrapper((ke2, kY0) => {
    /*!
    ...
    */
    (function(A) {
        if (typeof ke2 === "object" && typeof kY0 < "u") kY0.exports = A();
        ...
        return function G(Z, I, Y) {
            function J(F, V) {
                if (!I[F]) {
                    if (!Z[F]) {
                        ...
                        if (!V && K) return K(F, !0);
                        if (W) return W(F, !0);
                        var D = Error("Cannot find module '" + F + "'");
                        throw D.code = "MODULE_NOT_FOUND", D
                    }
                    var H = I[F] = {
                        exports: {}
                    };
                    Z[F][0].call(H.exports, function(C) {
                        var E = Z[F][1][C];
                        return J(E ? E : C)
                    }, H, H.exports, G, Z, I, Y)
                }
            ...
            1: [function(G, Z, I) {
                (function(Y) {
                    ...
                    if (J) {
                        ...
                        var z = Y.document.createElement("script");
                        z.onreadystatechange = function() {
                            C(), z.onreadystatechange = null, z.parentNode.removeChild(z), z = null
                        }, Y.document.documentElement.appendChild(z)
                    };
                    else W = function() {
                        setTimeout(C, 0)
                    };
            ...
            2: [function(G, Z, I) {
                ...
                function K(P) {
                    if (typeof P !== "function") throw TypeError("resolver must be a function");
                    if (this.state = V, this.queue = [], this.outcome = void 0, P !== J) E(this, P)
                }
                K.prototype.catch = function(P) {
                    return this.then(null, P)
                }, K.prototype.then = function(P, y) {
                    if (typeof P !== "function" && this.state === F || typeof y !== "function" && this.state === X) return this;
                    var v = new this.constructor(J);
                    if (this.state !== V) {
                        var x = this.state === F ? P : y;
                        H(v, x, this.outcome)
                    } else this.queue.push(new D(v, P, y));
                    return v
                };

                function D(P, y, v) {
                    if (this.promise = P, typeof y === "function") this.onFulfilled = y, this.callFulfilled = this.otherCallFulfilled;
                    if (typeof v === "function") this.onRejected = v, this.callRejected = this.otherCallRejected
                }
                D.prototype.callFulfilled = function(P) {
                    W.resolve(this.promise, P)
                }, D.prototype.otherCallFulfilled = function(P) {
                    H(this.promise, this.onFulfilled, P)
                }, D.prototype.callRejected = function(P) {
                    W.reject(this.promise, P)
                }, D.prototype.otherCallRejected = function(P) {
                ...
                function H(P, y, v) {
                    Y(function() {
                        ...
                        if (x === P) W.reject(P, TypeError("Cannot resolve promise with itself"));
                        else W.resolve(P, x)
                    })
                }
                W.resolve = function(P, y) {
                    var v = z(C, y);
                    if (v.status === "error") return W.reject(P, v.value);
                    var x = v.value;
                    if (x) E(P, x);
                    else {
                        P.state = F, P.outcome = y;
                        var p = -1,
                            u = P.queue.length;
                        while (++p < u) P.queue[p].callFulfilled(y)
                    }
                    return P
                }, W.reject = function(P, y) {
                    P.state = X, P.outcome = y;
                    var v = -1,
                        x = P.queue.length;
                    while (++v < x) P.queue[v].callRejected(y);
                    return P
                };
                ...
                function E(P, y) {
                    ...
                    var o = z(u);
                    if (o.status === "error") x(o.value)
                }

                function z(P, y) {
                    var v = {};
                    try {
                        v.value = P(y), v.status = "success"
                    } catch (x) {
                ...
                function q(P) {
                    var y = this;
                    if (Object.prototype.toString.call(P) !== "[object Array]") return this.reject(TypeError("must be an array"));
                    var v = P.length,
                        x = !1;
                    ...
                    function k(d, QA) {
                        y.resolve(d).then(IA, function(HA) {
                            if (!x) x = !0, W.reject(l, HA)
                        });

                        function IA(HA) {
                            if (p[QA] = HA, ++u === v && !x) x = !0, W.resolve(l, p)
                        }
                    }
                }
                K.race = R;
            ...
            4: [function(G, Z, I) {
                var Y = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function(dA) {
                    return typeof dA
                } : function(dA) {
                    return dA && typeof Symbol === "function" && dA.constructor === Symbol && dA !== Symbol.prototype ? "symbol" : typeof dA
                };

                function J(dA, YA) {
                    if (!(dA instanceof YA)) throw TypeError("Cannot call a class as a function")
                }
                ...
                function F() {
                    try {
                        ...
                        var dA = typeof openDatabase < "u" && /(Safari|iPhone|iPad|iPod)/.test(navigator.userAgent) && !/Chrome/.test(navigator.userAgent) && !/BlackBerry/.test(navigator.platform),
                            YA = typeof fetch === "function" && fetch.toString().indexOf("[native code") !== -1;
                        return (!dA || YA) && typeof indexedDB < "u" && typeof IDBKeyRange < "u"
                    } catch (ZA) {
                        return !1
                    }
                }
                ...
                function V(dA, YA) {
                    dA = dA || [], YA = YA || {};
                    try {
                        return new Blob(dA, YA)
                    } catch (mA) {
                        if (mA.name !== "TypeError") throw mA;
                        var ZA = typeof BlobBuilder < "u" ? BlobBuilder : typeof MSBlobBuilder < "u" ? MSBlobBuilder : typeof MozBlobBuilder < "u" ? MozBlobBuilder : WebKitBlobBuilder,
                            jA = new ZA;
                        for (var xA = 0; xA < dA.length; xA += 1) jA.append(dA[xA]);
                        return jA.getBlob(YA.type)
                    }
                }
                ...
                function H(dA, YA, ZA) {
                    if (typeof YA === "function") dA.then(YA);
                    if (typeof ZA === "function") dA.catch(ZA)
                }

                function C(dA) {
                    if (typeof dA !== "string") console.warn(dA + " used as a key, but it is not a string."), dA = String(dA);
                    return dA
                }
                ...
                function y(dA) {
                    var YA = dA.length,
                        ...
                        jA = new Uint8Array(ZA);
                    for (var xA = 0; xA < YA; xA++) jA[xA] = dA.charCodeAt(xA);
                    return ZA
                }

                function v(dA) {
                    return new K(function(YA) {
                        var ZA = dA.transaction(z, P),
                            jA = V([""]);
                        ZA.objectStore(z).put(jA, "key"), ZA.onabort = function(xA) {
                ...
                function x(dA) {
                    if (typeof w === "boolean") return K.resolve(w);
                    return v(dA).then(function(YA) {
                        return w = YA, w
                    })
                }
                ...
                function p(dA) {
                    var YA = N[dA.name],
                        ZA = {};
                    if (ZA.promise = new K(function(jA, xA) {
                            ZA.resolve = jA, ZA.reject = xA
                ...
                function o(dA, YA) {
                    var ZA = N[dA.name],
                        jA = ZA.deferredOperations.pop();
                    if (jA) return jA.reject(YA), jA.promise
                }

                function l(dA, YA) {
                    return new K(function(ZA, jA) {
                        if (N[dA.name] = N[dA.name] || NA(), dA.db)
                            if (YA) p(dA), dA.db.close();
                            else return ZA(dA.db);
                        var xA = [dA.name];
                        if (YA) xA.push(dA.version);
                        var mA = X.open.apply(X, xA);
                        if (YA) mA.onupgradeneeded = function(E1) {
                            var S1 = mA.result;
                            try {
                                if (S1.createObjectStore(dA.storeName), E1.oldVersion <= 1) S1.createObjectStore(z)
                            } catch (P1) {
                                if (P1.name === "ConstraintError") console.warn('The database "' + dA.name + '" has been upgraded from version ' + E1.oldVersion + " to version " + E1.newVersion + ', but the storage "' + dA.storeName + '" already exists.');
                                else throw P1
                            }
                        };
                        mA.onerror = function(E1) {
                            E1.preventDefault(), jA(mA.error)
                        }, mA.onsuccess = function() {
                            var E1 = mA.result;
                            E1.onversionchange = function(S1) {
                                S1.target.close()
                            }, ZA(E1), u(dA)
                        }
                    })
                ...
                function QA(dA, YA) {
                    if (!dA.db) return !0;
                    var ZA = !dA.db.objectStoreNames.contains(dA.storeName),
                        jA = dA.version < dA.db.version,
                        xA = dA.version > dA.db.version;
                    if (jA) {
                        if (dA.version !== YA) console.warn('The database "' + dA.name + `" can't be downgraded from version ` + dA.db.version + " to version " + dA.version + ".");
                        dA.version = dA.db.version
                    }
                    if (xA || ZA) {
                        if (ZA) {
                            var mA = dA.db.version + 1;
                            if (mA > dA.version) dA.version = mA
                        }
                        return !0
                    }
                    return !1
                ...
                function IA(dA) {
                    return new K(function(YA, ZA) {
                        var jA = new FileReader;
                        jA.onerror = ZA, jA.onloadend = function(xA) {
                            var mA = btoa(xA.target.result || "");
                            YA({
                                __local_forage_encoded_blob: !0,
                                data: mA,
                                type: dA.type
                            })
                        }, jA.readAsBinaryString(dA)
                    })
                }

                function HA(dA) {
                    var YA = y(atob(dA.data));
                    return V([YA], {
                        type: dA.type
                    })
                }
                ...
                function wA(dA) {
                    return dA && dA.__local_forage_encoded_blob
                }

                function KA(dA) {
                    var YA = this,
                        ZA = YA._initReady().then(function() {
                            var jA = N[YA._dbInfo.name];
                            if (jA && jA.dbReady) return jA.dbReady
                        });
                    return H(ZA, dA, dA), ZA
                }
                ...
                function SA(dA) {
                    ...
                    return dA.db = null, k(dA).then(function(mA) {
                        if (dA.db = mA, QA(dA)) return d(dA);
                        return mA
                    }).then(function(mA) {
                        dA.db = YA.db = mA;
                        for (var E1 = 0; E1 < ZA.length; E1++) ZA[E1]._dbInfo.db = mA
                    }).catch(function(mA) {
                        throw o(dA, mA), mA
                    })
                }

                function sA(dA, YA, ZA, jA) {
                    if (jA === void 0) jA = 1;
                    try {
                        var xA = dA.db.transaction(dA.storeName, YA);
                        ZA(null, xA)
                    } catch (mA) {
                        if (jA > 0 && (!dA.db || mA.name === "InvalidStateError" || mA.name === "NotFoundError")) return K.resolve().then(function() {
                            if (!dA.db || mA.name === "NotFoundError" && !dA.db.objectStoreNames.contains(dA.storeName) && dA.version <= dA.db.version) {
                                if (dA.db) dA.version = dA.db.version + 1;
                                return d(dA)
                            }
                        }).then(function() {
                            return SA(dA).then(function() {
                                sA(dA, YA, ZA, jA - 1)
                            })
                        }).catch(ZA);
                        ZA(mA)
                ...
                function qA(dA) {
                    ...
                    var xA = N[ZA.name];
                    if (!xA) xA = NA(), N[ZA.name] = xA;
                    if (xA.forages.push(YA), !YA._initReady) YA._initReady = YA.ready, YA.ready = KA;
                    var mA = [];

                    function E1() {
                        return K.resolve()
                    }
                    for (var S1 = 0; S1 < xA.forages.length; S1++) {
                        var P1 = xA.forages[S1];
                        if (P1 !== YA) mA.push(P1._initReady().catch(E1))
                    }
                    var c1 = xA.forages.slice(0);
                    return K.all(mA).then(function() {
                        return ZA.db = xA.db, k(ZA)
                    }).then(function(l1) {
                        if (ZA.db = l1, QA(ZA, YA._defaultConfig.version)) return d(ZA);
                        return l1
                    }).then(function(l1) {
                        ZA.db = xA.db = l1, YA._dbInfo = ZA;
                        for (var I0 = 0; I0 < c1.length; I0++) {
                            var e0 = c1[I0];
                            if (e0 !== YA) e0._dbInfo.db = ZA.db, e0._dbInfo.version = ZA.version
                        }
                    })
                }
                            ...
                            sA(ZA._dbInfo, R, function(E1, S1) {
                                ...
                                try {
                                    var P1 = S1.objectStore(ZA._dbInfo.storeName),
                                        c1 = P1.get(dA);
                                    c1.onsuccess = function() {
                                        var l1 = c1.result;
                                        if (l1 === void 0) l1 = null;
                                        if (wA(l1)) l1 = HA(l1);
                                        xA(l1)
                                    }, c1.onerror = function() {
                                        mA(c1.error)
