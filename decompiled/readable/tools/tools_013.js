...
async function Fp5(A, Q, B, G, Z, I, Y, J) {
    ...
    if (V && typeof V === "object") {
        if (A.name === READ_TOOL_NAME && "file_path" in V) C.file_path = String(V.file_path);
        else if ((A.name === EDIT_TOOL_NAME || A.name === WRITE_TOOL_NAME) && "file_path" in V) C.file_path = String(V.file_path);
        else if (A.name === BASH_TOOL_NAME && "command" in V) {
            let q = V;
    ...
    let z = {};
    if (A.name === BASH_TOOL_NAME && "command" in V) {
        let q = V;
        z = {
    ...
    try {
        ...
        if (pW1(R), q.data && typeof q.data === "object") {
            let u = {};
            if (A.name === READ_TOOL_NAME && "content" in q.data) {
                if ("file_path" in V) u.file_path = String(V.file_path);
                u.content = String(q.data.content)
            }
            if ((A.name === EDIT_TOOL_NAME || A.name === WRITE_TOOL_NAME) && "file_path" in V) {
                if (u.file_path = String(V.file_path), A.name === EDIT_TOOL_NAME && "diff" in q.data) u.diff = String(q.data.diff);
                if (A.name === WRITE_TOOL_NAME && "content" in V) u.content = String(V.content)
            }
            if (A.name === BASH_TOOL_NAME && "command" in V) {
                let o = V;
...
function Fb2(A, Q) {
    let B = j0({
        ...
        G = Q.message.content.find((W) => {
            if (W.type !== "tool_use" || W.name !== TASK_TOOL_NAME) return !1;
            let X = W.input;
