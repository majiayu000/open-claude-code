 ...
 * 本文件中使用的混淆变量:
 *   L        (52次) = lazyLoader(fn) - Lazy module loader
 *   SQ       (17次) = sandboxDebug(msg, options) - Debug logging for sandbox
 *   Fb1      (4次) = defaultConnectionHandler() - Default SOCKS connection handler
 *   Xb1      (4次) = SocksStatus enum - SOCKS response status codes
 *   niA      (4次) = baseSlice() - Base array slice
 *   aiA      (4次) = hasUnicode() - Check for unicode chars
 *   siA      (4次) = baseEach() - Base forEach implementation
 *   x6B      (3次) = Socks5Server class
 *   y6B      (3次) = SocksCommand enum - SOCKS commands (connect/bind/udp)
 *   u6B      (3次) = baseFindIndex() - Base find index implementation
 *   d6B      (3次) = baseIsNaN() - Check if value is NaN
 *   p6B      (3次) = strictIndexOf() - Strict array indexOf
 *   i6B      (3次) = baseIndexOf() - Base indexOf with NaN support
 *   a6B      (3次) = arrayIncludes() - Check if array includes value
 *   o6B      (3次) = isFlattenable() - Check if value can be flattened
 *   A5B      (3次) = baseFlatten() - Flatten array implementation
 *   B5B      (3次) = flatten() - Flatten array one level
 *   Y5B      (3次) = castSlice() - Cast to slice with defaults
 *   W5B      (3次) = asciiToArray() - Convert ASCII to array
 *   E5B      (3次) = unicodeToArray() - Convert unicode to array
 *   U5B      (3次) = stringToArray() - Convert string to array
 *   w5B      (3次) = createCaseFirst() - Create first char case function
 *   N5B      (3次) = upperFirst() - Uppercase first char
 *   o5B      (3次) = baseSet() - Base set property on object
 *   b6B      (2次) = socks5ServerModule - SOCKS5 server implementation
 *   Re8      (2次) = createSocksServer() - Create SOCKS5 server
 *   Me8      (2次) = Socks5Connection class - Handles SOCKS5 connections
 *   Ve8      (2次) = http.createServer
 *   Ke8      (2次) = http.request
 *   De8      (2次) = https.request
 * ======================================================
 */
...
var D9 = /* BASH_TOOL_NAME */ "Bash";

/* SQ: (message: string, options?: {level: "info"|"warn"|"error"}) => void */
/* sandboxDebug(msg, options) - Debug logging for sandbox */
function SQ(A, Q) {
...
/* createProxyServer(config) - Create HTTP/HTTPS proxy server */
function P6B(A) {
    let Q = Ve8();
...
var b6B = U((mj7, v6B) => {
    ...
    /* Fb1 */
/* defaultConnectionHandler() - Default SOCKS connection handler */
function Fb1(A, Q) {
...
var x6B = class {
    ...
    /* Re8 */
/* createSocksServer() - Create SOCKS5 server */
function Re8(A) {
        let Q = new x6B;
