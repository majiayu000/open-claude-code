 ...
 *
 * 原始文件: prompts_003.js
 * 处理时间: 2025-12-09T03:37:25.267Z
 *
 * 注意: 此文件保留了100%的原始代码逻辑
 * 通过注释解释混淆的变量名和函数名
 *
 * ==================== 变量索引 ====================
 * M8         ( 15x) = shellEscape(args) - Shell escape utility
 * D9         (  6x) = BASH_TOOL = "Bash"
 * BwA        (  6x) = fs.existsSync
 * GA         (  5x) = esmImport(module) - ESM import helper
 * id1        (  4x) = hasHeredoc(cmd) - Check if command has heredoc
 * nd1        (  4x) = isOperator(token, op) - Check if token is operator
 * sd1        (  4x) = path.join
 * rd1        (  4x) = getShellRcFile() - Get .bashrc/.zshrc path
 * RJ         (  4x) = isClaudeCodeRemote() - Check if remote execution
 * KoA        (  3x) = getMaxTimeout() - Returns 600000ms (10 minutes)
 * LGA        (  3x) = getDefaultTimeout() - Returns 120000ms (2 minutes)
 * Ke         (  3x) = getMaxOutputLength() - Returns 30000 characters
 * TSB        (  3x) = extractTokensRange() - Extract tokens from range
 * DoA        (  3x) = os module
 * s8         (  2x) = TASK_TOOL = "Task"
 * tI         (  2x) = TODO_READ_TOOL object
 * oM6        (  2x) = getSandboxHint() - Returns sandbox usage hint text
 * tM6        (  2x) = getGitCommitInstructions() - Returns git commit instructions
 * rM6        (  2x) = getGitConfig() - Returns git commit/pr config object
 * NSB        (  2x) = getReadToolDescription() - Read tool description
 * OSB        (  2x) = shouldAddDevNull() - Check if need /dev/null redirect
 * MSB        (  2x) = wrapBashCommand() - Wrap bash command with escaping
 * PSB        (  2x) = addDevNullToCommand() - Add /dev/null redirect
 * RSB        (  2x) = bashCommandHelpers module loader
 * eM6        (  2x) = hasMultilineString(cmd) - Check for multiline strings
 * AO6        (  2x) = hasInputRedirect(cmd) - Check for input redirect
 * BO6        (  2x) = isEnvAssignment(token) - Check env assignment pattern
 * GO6        (  2x) = isCommandSeparator(op) - Check command separator (&&, ||, ;)
 * QO6        (  2x) = findPipeIndex(tokens) - Find first pipe in tokens
 * ZO6        (  2x) = fs.statSync
 * IO6        (  2x) = fs.mkdirSync
 * YO6        (  2x) = fs.realpathSync
 * JO6        (  2x) = child_process.execSync
 * WO6        (  2x) = child_process.execFile
 * XO6        (  2x) = getRipgrepCommand() - Get rg command with args
 * FO6        (  2x) = getShellSnapshotScript() - Get shell env snapshot
 * CD         (  1x) = GLOB_TOOL = "Glob"
 * uY         (  1x) = GREP_TOOL = "Grep"
 * g5         (  1x) = READ_TOOL = "Read"
 * R5         (  1x) = EDIT_TOOL = "Edit"
 * ... 还有 4 个变量 ...
 * ===================================================
 */
...
/* Signature: () => string // Read tool description */
function NSB() {
...
/* Signature: () => string // Bash tool description */
function LSB() {
...
/* Signature: (command: string) => boolean */
function id1(A) {
...
/* Signature: (command: string, addDevNull?: boolean) => string */
function MSB(A, Q = !0) {
    if (id1(A) || eM6(A)) {
        let G = `'${A.replace(/'/g,`'"'"'`)}'`;
        if (id1(A)) return G;
...
/* Signature: (token: any, op?: string) => boolean */
function nd1(A, Q) {
