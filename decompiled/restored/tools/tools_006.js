 ...
 *
 * 原始文件: tools_006.js
 * 处理时间: 2025-12-09T03:37:25.551Z
 *
 * 注意: 此文件保留了100%的原始代码逻辑
 * 通过注释解释混淆的变量名和函数名
 *
 * ==================== 变量索引 ====================
 * thinking   ( 10x) = THINKING content type
 * IE         (  9x) = EventSourceIterator class - SSE event processing
 * uY         (  6x) = GREP_TOOL = "Grep"
 * Kt         (  5x) = MessageStream class - Streaming message handling
 * CD         (  4x) = GLOB_TOOL = "Glob"
 * g5         (  4x) = READ_TOOL = "Read"
 * vX         (  4x) = WEB_FETCH_TOOL = "WebFetch"
 * O_         (  4x) = WEB_SEARCH_TOOL = "WebSearch"
 * message_start (  4x) = MESSAGE_START event type
 * message_stop (  4x) = MESSAGE_STOP event type
 * server_tool_use (  4x) = SERVER_TOOL_USE type
 * qGB        (  3x) = SYSTEM_PROMPT_BASE = "You are Claude Code..."
 * message_delta (  3x) = MESSAGE_DELTA event type
 * content_block_start (  3x) = CONTENT_BLOCK_START event
 * content_block_delta (  3x) = CONTENT_BLOCK_DELTA event
 * content_block_stop (  3x) = CONTENT_BLOCK_STOP event
 ...
 * XGB        (  2x) = EventDecoder class - Event decoding
 * text_delta (  2x) = TEXT_DELTA event type
 * tool_use   (  1x) = TOOL_USE content type
 * mcp_tool_use (  1x) = MCP_TOOL_USE type
 * ===================================================
 */
...
var wnA, Bf1 = (A, Q, B) => {
    ...
    a06, WGB, gb = (A) => {
        ...
        if (A.headers) A.headers = Object.fromEntries((A.headers instanceof Headers ? [...A.headers] : Object.entries(A.headers)).map(([Q, B]) => [Q, Q.toLowerCase() === /* HEADER_API_KEY */ "x-api-key" || Q.toLowerCase() === "authorization" || Q.toLowerCase() === "cookie" || Q.toLowerCase() === "set-cookie" ? "***" : B]));
