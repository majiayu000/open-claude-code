 ...
 * 本文件中使用的混淆变量:
 *   M8       (15次) = shellEscape() - Shell escape utility
 *   L        (11次) = lazyLoader(fn) - Lazy module loader
 *   D9       (6次) = BASH_TOOL_NAME = "Bash"
 *   BwA      (6次) = fs.existsSync
 *   GA       (5次) = esmImport(module) - ESM import helper
 *   id1      (4次) = hasHeredoc() - Check if command has heredoc
 *   nd1      (4次) = isOperator() - Check if token is operator
 *   sd1      (4次) = path.join
 *   rd1      (4次) = getShellRcFile() - Get shell rc file path (.bashrc/.zshrc)
 *   TSB      (3次) = extractTokensRange() - Extract tokens from range
 *   KoA      (3次) = getMaxTimeout() - Returns max timeout (600000ms)
 *   LGA      (3次) = getDefaultTimeout() - Returns default timeout (120000ms)
 *   Ke       (3次) = getMaxOutputLength() - Returns max output length (30000)
 *   DoA      (3次) = os module
 *   s8       (2次) = TASK_TOOL_NAME = "Task"
 *   tI       (2次) = TODO_READ_TOOL = TodoRead tool object
 *   NSB      (2次) = getReadToolDescription() - Returns Read tool description
 *   OSB      (2次) = shouldAddDevNull() - Check if need /dev/null redirect
 *   MSB      (2次) = wrapBashCommand() - Wrap bash command with proper escaping
 *   PSB      (2次) = addDevNullToCommand() - Add /dev/null redirect to command
 *   RSB      (2次) = bashCommandHelpers module loader
 *   oM6      (2次) = getSandboxHint() - Returns sandbox usage hint
 *   tM6      (2次) = getGitCommitInstructions() - Returns git commit instructions
 *   rM6      (2次) = getGitConfig() - Returns git commit/pr config
 *   eM6      (2次) = hasMultilineString() - Check for multiline strings
 *   AO6      (2次) = hasInputRedirect() - Check for input redirect
 *   BO6      (2次) = isEnvAssignment() - Check if token is env assignment
 *   GO6      (2次) = isCommandSeparator() - Check if operator separates commands
 *   QO6      (2次) = findPipeIndex() - Find first pipe in tokens
 *   ZO6      (2次) = fs.statSync
 * ======================================================
 */
...
/* NSB */
/* getReadToolDescription() - Returns Read tool description */
function NSB() {
...
/* Ke */
/* getMaxOutputLength() - Returns max output length (30000) */
function Ke() {
...
/* LGA */
/* getDefaultTimeout() - Returns default timeout (120000ms) */
function LGA() {
    return VoA()
}

/* KoA */
/* getMaxTimeout() - Returns max timeout (600000ms) */
function KoA() {
    return NSB()
}

/* rM6 */
/* getGitConfig() - Returns git commit/pr config */
function rM6() {
...
/* oM6 */
/* getSandboxHint() - Returns sandbox usage hint */
function oM6() {
...
/* getBashToolDescription() - Returns Bash tool description */
function LSB() {
...
Usage notes:
  ...
  - You can use the \`run_in_background\` parameter to run the command in the background, which allows you to continue working while the command runs. You can monitor the output using the ${D9} tool as it becomes available. You do not need to use '&' at the end of the command when using this parameter.
  ${oM6()}
  - Avoid using Bash with the \`find\`, \`grep\`, \`cat\`, \`head\`, \`tail\`, \`sed\`, \`awk\`, or \`echo\` commands, unless explicitly instructed or when these commands are truly necessary for the task. Instead, always prefer using the dedicated tools for these commands:
    - File search: Use ${CD} (NOT find or ls)
    - Content search: Use ${uY} (NOT grep or rg)
    - Read files: Use ${g5} (NOT cat/head/tail)
    - Edit files: Use ${R5} (NOT sed/awk)
    - Write files: Use ${bX} (NOT echo >/cat <<EOF)
    - Communication: Output text directly (NOT echo/printf)
  - When issuing multiple commands:
    - If the commands are independent and can run in parallel, make multiple ${D9} tool calls in a single message. For example, if you need to run "git status" and "git diff", send a single message with two ${D9} tool calls in parallel.
    - If the commands depend on each other and must run sequentially, use a single ${D9} call with '&&' to chain them together (e.g., \`git add . && git commit -m "message" && git push\`). For instance, if one operation must complete before another starts (like mkdir before cp, Write before Bash for git operations, or git add before git commit), run these operations sequentially instead.
...
/* tM6 */
/* getGitCommitInstructions() - Returns git commit instructions */
function tM6() {
    let {
        commit: Q,
        pr: B
...
1. You can call multiple tools in a single response. When multiple independent pieces of information are requested and all commands are likely to succeed, run multiple tool calls in parallel for optimal performance. run the following bash commands in parallel, each using the ${D9} tool:
...
- NEVER use the ${tI.name} or ${s8} tools
...
1. You can call multiple tools in a single response. When multiple independent pieces of information are requested and all commands are likely to succeed, run multiple tool calls in parallel for optimal performance. run the following bash commands in parallel using the ${D9} tool, in order to understand the current state of the branch since it diverged from the main branch:
   - Run a git status command to see all untracked files
   - Run a git diff command to see both staged and unstaged changes that will be committed
...
Important:
- DO NOT use the ${tI.name} or ${s8} tools
...
/* hasHeredoc() - Check if command has heredoc */
function id1(A) {
...
/* hasMultilineString() - Check for multiline strings */
function eM6(A) {
    let Q = /'(?:[^'\\]|\\.)*\n(?:[^'\\]|\\.)*'/,
        B = /"(?:[^"\\]|\\.)*\n(?:[^"\\]|\\.)*"/;
...
/* wrapBashCommand() - Wrap bash command with proper escaping */
function MSB(A, Q = !0) {
    if (id1(A) || eM6(A)) {
        let G = `'${A.replace(/'/g,`'"'"'`)}'`;
        if (id1(A)) return G;
...
/* hasInputRedirect() - Check for input redirect */
function AO6(A) {
    return /(?:^|[\s;&|])<(?![<(])\s*\S+/.test(A)
}

/* OSB: (command: string) => boolean */
/* shouldAddDevNull() - Check if need /dev/null redirect */
function OSB(A) {
    if (id1(A)) return !1;
    if (AO6(A)) return !1;
    return !0
...
/* addDevNullToCommand() - Add /dev/null redirect to command */
function PSB(A) {
...
/* QO6 */
/* findPipeIndex() - Find first pipe in tokens */
function QO6(A) {
...
/* TSB */
/* extractTokensRange() - Extract tokens from range */
function TSB(A, Q, B) {
    let G = [],
        Z = !1;
    for (let I = Q; I < B; I++) {
...
/* BO6 */
/* isEnvAssignment() - Check if token is env assignment */
function BO6(A) {
    return /^[A-Za-z_][A-Za-z0-9_]*=/.test(A)
}

/* GO6 */
/* isCommandSeparator() - Check if operator separates commands */
function GO6(A) {
    return A === "&&" || A === "||" || A === ";"
}
...
/* XO6 */
/* getRipgrepCommand() - Get rg command with args */
function XO6() {
    let A = C9A(),
        Q = M8([A.rgPath]),
...
/* rd1 */
/* getShellRcFile() - Get shell rc file path (.bashrc/.zshrc) */
function rd1(A) {
...
/* FO6 */
/* getShellSnapshotScript() - Get shell env snapshot script */
function FO6(A) {
