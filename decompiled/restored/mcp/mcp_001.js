 ...
 *
 * 原始文件: mcp_001.js
 * 处理时间: 2025-12-09T03:37:24.932Z
 *
 * 注意: 此文件保留了100%的原始代码逻辑
 * 通过注释解释混淆的变量名和函数名
 *
 * ==================== 变量索引 ====================
 * HX         ( 11x) = globalThis - Global object reference
 * xz         (  8x) = nativeMethod(obj, name) - Native method lookup
 * HV         (  8x) = Symbol - Symbol reference
 * Ks         (  6x) = Map - Map constructor
 * _u         (  6x) = Array - Array constructor
 * j7         (  6x) = isArray() - Check if array
 * TY         (  4x) = isObject() - Check if object
 * pG         (  3x) = esmExport(obj, key) - ESM export binding
 * Tj         (  3x) = isArrayLike() - Check array-like
 * px         (  3x) = isArguments() - Check if arguments
 * SN         (  3x) = keys() - Get object keys
 * jBA        (  3x) = arrayPush() - Array push helper
 * xu         (  3x) = isIndex() - Check if valid index
...
function D_A(A) {
    ...
    if (A?.startsWith(/* MODEL_SONNET_3_5 */ "claude-3-5-sonnet")) return process.env.VERTEX_REGION_CLAUDE_3_5_SONNET || XR();
    ...
    if (A?.startsWith(/* MODEL_SONNET */ "claude-sonnet-4-5")) return process.env.VERTEX_REGION_CLAUDE_4_5_SONNET || XR();
