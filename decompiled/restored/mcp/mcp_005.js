 ...
 * LlA        (  3x) = baseForOwn() - Base for own properties
 * jj         (  1x) = baseIteratee() - Base iteratee
...
function YQB(A) {
    ...
    let Q = A.toLowerCase();
    if (Q.includes(/* MODEL_SONNET */ "claude-sonnet-4-5") && Q.includes("[1m]")) return "Sonnet 4.5 (with 1M context)";
    if (Q.includes(/* MODEL_SONNET */ "claude-sonnet-4-5")) return "Sonnet 4.5";
    if (Q.includes("claude-sonnet-4") && Q.includes("[1m]")) return "Sonnet 4 (with 1M context)";
    if (Q.includes("claude-sonnet-4")) return "Sonnet 4";
    if (Q.includes(/* MODEL_OPUS */ "claude-opus-4-5")) return "Opus 4.5";
    ...
    if (Q.includes(/* MODEL_SONNET_3_5 */ "claude-3-5-sonnet")) return "Claude 3.5 Sonnet";
...
var iEA = L(() => {
    ...
    gEA = {
        ...
        foundry: /* MODEL_SONNET_3_5 */ "claude-3-5-sonnet"
    }, mEA = {
        ...
        foundry: /* MODEL_SONNET */ "claude-sonnet-4-5"
    }, cEA = {
        ...
        foundry: /* MODEL_OPUS */ "claude-opus-4-5"
    }
});
...
function ZU() {
    ...
    return !(A || G || (Z === /* ENV_API_KEY */ "ANTHROPIC_API_KEY" || Z === "apiKeyHelper") && !V0(process.env.CLAUDE_CODE_REMOTE))
}

function Kc() {
    if (process.env.ANTHROPIC_AUTH_TOKEN) return {
        source: /* ENV_AUTH_TOKEN */ "ANTHROPIC_AUTH_TOKEN",
        hasToken: !0
    };
...
function vw(A = {}) {
    if (_E0() && process.env.ANTHROPIC_API_KEY) return {
        ...
        source: /* ENV_API_KEY */ "ANTHROPIC_API_KEY"
    };
    if (V0(!1)) {
        let G = IE1();
        if (G) return {
