 ...
 * tI       (  9x) TODO_READ_TOOL object
 * s8       (  8x) TASK_TOOL = "Task"
 * dJ       (  4x) ASK_USER_QUESTION_TOOL = "AskUserQuestion"
 * V0       (  4x) parseBoolean(value) - Parse bool env
 * D9       (  3x) BASH_TOOL = "Bash"
 * vX       (  3x) WEB_FETCH_TOOL = "WebFetch"
 * g5       (  2x) READ_TOOL = "Read"
 * R5       (  2x) EDIT_TOOL = "Edit"
 * RJ       (  2x) isClaudeCodeRemote() - Check remote
 * ZI       (  2x) getFeatureFlag(name, scope, default)
 * CD       (  1x) GLOB_TOOL = "Glob"
 * uY       (  1x) GREP_TOOL = "Grep"
 * bX       (  1x) WRITE_TOOL = "Write"
 * Pq       (  1x) SKILL_TOOL = "Skill"
 * WY9      (  1x) AGENT_BASE_PROMPT = "You are an agent for Claude Cod...
 * _nA      (  1x) getSystemPrompt(options) - Get appropriate system pr...
 * Hf1      (  1x) systemPromptLoader - Lazy loader for prompts
 * LW       (  1x) getSmallFastModel() - Returns haiku model
 * ZQ       (  1x) execGit(cmd, args) - Execute git command
 * SnA      (  1x) getTodayDate() - Returns "YYYY-MM-DD"
 * XT       (  1x) noOpFunction() - Empty function
 * Uf1      (  1x) CLAUDE_CODE_GUIDE_AGENT = "claude-code-guide"
 * ======================================================
 */
...
Use the ${s8} tool with subagent_type='${Uf1}' to get accurate information from the official Claude Code and Claude Agent SDK documentation.

${Y!==null?"":`# Tone and style
- Only use emojis if the user explicitly requests it. Avoid using emojis in all communication unless asked.
- Your output will be displayed on a command line interface. Your responses should be short and concise. You can use Github-flavored markdown for formatting, and will be rendered in a monospace font using the CommonMark specification.
- Output text to communicate with the user; all text you output outside of tool use is displayed to the user. Only use tools to complete tasks. Never use tools like ${D9} or code comments as means to communicate with the user during the session.
...
I'm now going to run the build using ${D9}.
...
# Tool usage policy${W.has(s8)?`
- When doing file search, prefer to use the ${s8} tool in order to reduce context usage.
- You should proactively use the ${s8} tool with specialized agents when the task at hand matches the agent's description.
${V}`:""}${W.has(vX)?`
- When ${vX} returns a message about a redirect to a different host, you should immediately make a new ${vX} request with the redirect URL provided in the response.`:""}
- You can call multiple tools in a single response. If you intend to call multiple tools and there are no dependencies between them, make all independent tool calls in parallel. Maximize use of parallel tool calls where possible to increase efficiency. However, if some tool calls depend on previous calls to inform dependent values, do NOT call these tools in parallel and instead call them sequentially. For instance, if one operation must complete before another starts, run these operations sequentially instead. Never use placeholders or guess missing parameters in tool calls.
- If the user specifies that they want you to run tools "in parallel", you MUST send a single message with multiple tool use content blocks. For example, if you need to launch multiple agents in parallel, send a single message with multiple ${s8} tool calls.
- Use specialized tools instead of bash commands when possible, as this provides a better user experience. For file operations, use dedicated tools: ${g5} for reading files instead of cat/head/tail, ${R5} for editing instead of sed/awk, and ${bX} for creating files instead of cat with heredoc or echo redirection. Reserve bash tools exclusively for actual system commands and terminal operations that require shell execution. NEVER use bash echo or other command-line tools to communicate thoughts, explanations, or instructions to the user. Output all communication directly in your response text instead.
- VERY IMPORTANT: When exploring the codebase to gather context or to answer a question that is not a needle query for a specific file/class/function, it is CRITICAL that you use the ${s8} tool with subagent_type=${Sq.agentType} instead of running search commands directly.
<example>
user: Where are errors from the client handled?
assistant: [Uses the ${s8} tool with subagent_type=${Sq.agentType} to find the files that handle client errors instead of using ${CD} or ${uY} directly]
</example>
<example>
user: What is the codebase structure?
assistant: [Uses the ${s8} tool with subagent_type=${Sq.agentType}]
</example>
...
This is a BLOCKING REQUIREMENT - like how you must use ${g5} before ${R5}.
...
Use this command via ${D9} when you need to discover, inspect, or invoke MCP tools.
...
var yk3, BH9 = "IMPORTANT: Assist with authorized security testing, defensive security, CTF challenges, and educational contexts. Refuse requests for destructive techniques, DoS attacks, mass targeting, supply chain compromise, or detection evasion for malicious purposes. Dual-use security tools (C2 frameworks, credential testing, exploit development) require clear authorization context: pentesting engagements, CTF competitions, security research, or defensive use cases.",
    ...
    WY9 = "You are an agent for Claude Code, Anthropic's official CLI for Claude. Given the user's message, you should use the tools available to complete the task. Do what has been asked; nothing more, nothing less. When you complete the task simply respond with a detailed writeup.";
var $n = L(() => {
    f5();
    ED();
    R2();
