 ...
 *
 * 原始文件: tools_009.js
 * 处理时间: 2025-12-09T03:37:25.576Z
 *
 * 注意: 此文件保留了100%的原始代码逻辑
 * 通过注释解释混淆的变量名和函数名
 *
 * ==================== 变量索引 ====================
 * tool_use   (  9x) = TOOL_USE content type
...
function N00(A, Q, B) {
    ...
    if (A instanceof Error && A.message.toLowerCase().includes(/* HEADER_API_KEY */ "x-api-key")) {
        let {
            source: G
        } = vw();
        return WY({
            error: "authentication_failed",
            content: G === /* ENV_API_KEY */ "ANTHROPIC_API_KEY" || G === "apiKeyHelper" ? s21 : a21
        })
    }
...
function bZ2(A) {
    ...
    if (A instanceof Error && A.message.toLowerCase().includes(/* HEADER_API_KEY */ "x-api-key")) return "invalid_api_key";
