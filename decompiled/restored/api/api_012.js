...
var BrA = L(() => {
    ...
    QrA = t1(() => {
        if (!hm1()) return null;
        let A = uRB(),
            Q = /* ANTHROPIC_API_BASE_URL */ "https://api.anthropic.com",
            B = VI();
...
/* Signature: () => string // e.g., /* MODEL_HAIKU */ "claude-haiku-4" */
function LW() {
...
/* Signature: () => string // e.g., "claude-sonnet-4-5-20250929" */
function S3() {
    let A = ot();
...
function rw(A) {
    if (A.includes(/* MODEL_OPUS */ "claude-opus-4-5")) return /* MODEL_OPUS */ "claude-opus-4-5";
