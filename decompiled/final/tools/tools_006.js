 ...
 * Kt       (  5x) MessageStream class
 * CD       (  4x) GLOB_TOOL = "Glob"
 * g5       (  4x) READ_TOOL = "Read"
 * vX       (  4x) WEB_FETCH_TOOL = "WebFetch"
 * O_       (  4x) WEB_SEARCH_TOOL = "WebSearch"
 * qGB      (  3x) SYSTEM_PROMPT = "You are Claude Code, Anthropic's of...
 * zGB      (  3x) MAX_INPUT_TOKENS = 180000
 * UGB      (  3x) TARGET_INPUT_TOKENS = 40000
 * e06      (  3x) READONLY_TOOLS = [Bash, Glob, Grep, Read, WebFetch, ...
 * AQ6      (  3x) EDIT_TOOLS = [Edit, Write, NotebookEdit]
 * knA      (  3x) getGitRemoteUrl() - Get git remote URL
 * XT       (  3x) noOpFunction() - Empty function
 * B7A      (  3x) noOpFunction2() - Empty function
 * D9       (  2x) BASH_TOOL = "Bash"
 * R5       (  2x) EDIT_TOOL = "Edit"
 * bX       (  2x) WRITE_TOOL = "Write"
 * M_       (  2x) NOTEBOOK_EDIT_TOOL = "NotebookEdit"
 * gGB      (  2x) TODO_WRITE_TOOL = "TodoWrite"
 * s8       (  2x) TASK_TOOL = "Task"
 * en       (  2x) AGENT_OUTPUT_TOOL = "AgentOutputTool"
 * tI       (  2x) TODO_READ_TOOL object
 * CGB      (  2x) WRITE_TOOL_DESCRIPTION - Write tool help text
 * QQ6      (  2x) SDK_SYSTEM_PROMPT = "You are Claude Code...running w...
 * BQ6      (  2x) AGENT_SYSTEM_PROMPT = "You are a Claude agent..."
 * XGB      (  2x) EventDecoder class
 * IQ6      (  2x) parseGitRemoteUrl(url) - Parse git remote URL
 * YQ6      (  2x) getCommitsBehind() - Get commits behind main branch
 * Cf1      (  2x) getMainBranch() - Get main branch name
 * SnA      (  2x) getTodayDate() - Returns "YYYY-MM-DD"
 * GQ6      (  2x) crypto.createHash
 * V0       (  2x) parseBoolean(value) - Parse bool env
 * Uf1      (  2x) CLAUDE_CODE_GUIDE_AGENT = "claude-code-guide"
 * JQ6      (  2x) DOCS_MAP_URL = "https://code.claude.com/docs/..."
 * Kf1      (  1x) GLOB_TOOL_DESCRIPTION - Glob tool help text
 * Df1      (  1x) getGrepDescription() - Returns Grep tool description
 * EGB      (  1x) getWebSearchDescription() - Returns WebSearch descri...
 * _nA      (  1x) getSystemPrompt(options) - Get appropriate system pr...
 * NGB      (  1x) getEmptyString() - Returns empty string
 * Hf1      (  1x) systemPromptLoader - Lazy loader for prompts
 * J6       (  1x) getProvider() - Returns current provider ("vertex", ...
 * MGB      (  1x) getRepoHash() - Get repository hash
 * ZI       (  1x) getFeatureFlag(name, scope, default)
 * ======================================================
 */
...
function Df1() {
  ...
  Usage:
  - ALWAYS use ${uY} for search tasks. NEVER invoke \`grep\` or \`rg\` as a ${D9} command. The ${uY} tool has been optimized for correct permissions and access.
  - Supports full regex syntax (e.g., "log.*Error", "function\\s+\\w+")
  - Filter files with glob parameter (e.g., "*.js", "**/*.tsx") or type parameter (e.g., "js", "py", "rust")
  - Output modes: "content" shows matching lines, "files_with_matches" shows only file paths (default), "count" shows match counts
  - Use ${s8} tool for open-ended searches requiring multiple rounds
...
- If this is an existing file, you MUST use the ${g5} tool first to read the file's contents. This tool will fail if you did not read the file first.
...
var wGB = L(() => {
    ...
    B7A();
    O9();
    e06 = [D9, CD, uY, g5, vX, O_], AQ6 = [R5, bX, M_]
});
...
var qGB = "You are Claude Code, Anthropic's official CLI for Claude.",
    QQ6 = "You are Claude Code, Anthropic's official CLI for Claude, running within the Claude Agent SDK.",
...
async /* MGB = getRepoHash() - Get repository hash */
...
async /* YQ6 = getCommitsBehind() - Get commits behind main branch */
...
**Approach:**
1. Use ${vX} to access the documentation maps:
   - Claude Code: ${JQ6}
   - Agent SDK: ${WQ6}
...
3. Fetch the specific documentation pages using ${vX}
4. Provide clear, actionable guidance based on the official documentation
5. Use ${O_} if you need additional context or the docs don't cover the topic
6. Reference local project files (CLAUDE.md, .claude/ directory, etc.) when relevant using ${g5}, ${CD}, and ${uY}
